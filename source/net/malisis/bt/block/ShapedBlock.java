...
public class ShapedBlock extends ProxyBlock
{
	...
	public int calcShape(World world, int x, int y, int z)
	{
		...
		if (west && bottom && fBottom == NONE && fWest == NONE && !east && !top)
			f = TOPEAST;
		else if (east && bottom && fBottom == NONE && fEast == NONE && !west
				&& !top)
			f = TOPWEST;
		else if (north && bottom && fBottom == NONE && fNorth == NONE && !south
				&& !top)
			f = TOPSOUTH;
		else if (south && bottom && fBottom == NONE && fSouth == NONE && !north
				&& !top)
			f = TOPNORTH;

		if (north && fNorth == NONE && east && fEast == NONE && !south && !west)
			f = SOUTHWEST;
		else if (north && fNorth == NONE && west && fWest == NONE && !south
				&& !east)
			f = SOUTHEAST;
		else if (south && fSouth == NONE && east && fEast == NONE && !north
				&& !west)
			f = NORTHWEST;
		else if (south && fSouth == NONE && west && fWest == NONE && !north
				&& !east)
			f = NORTHEAST;

		if (((east && fSouth == TOPWEST) || (south && fEast == TOPNORTH))
				&& bottom && !top && !north && !west)
			f = TOPNORTHWEST;
		else if (((east && fNorth == TOPWEST) || (north && fEast == TOPSOUTH))
				&& bottom && !top && !south && !west)
			f = TOPSOUTHWEST;
		else if (((west && fSouth == TOPEAST) || (south && fWest == TOPNORTH))
				&& bottom && !top && !north && !east)
			f = TOPNORTHEAST;
		else if (((west && fNorth == TOPEAST) || (north && fWest == TOPSOUTH))
				&& bottom && !top && !south && !east)
			f = TOPSOUTHEAST;

		else if ((fNorth == TOPWEST || fNorth == TOPNORTHWEST)
				&& (fWest == TOPNORTH || fWest == TOPNORTHWEST)
				&& fSouth == NONE && fEast == NONE
				&& (!top || fTop == NORTHWEST || fTop == TOPNORTHWEST) && south
				&& east)
			f = INVTOPNORTHWEST;
		else if ((fSouth == TOPWEST || fSouth == TOPSOUTHWEST)
				&& (fWest == TOPSOUTH || fWest == TOPSOUTHWEST)
				&& fNorth == NONE && fEast == NONE
				&& (!top || fTop == SOUTHWEST || fTop == TOPSOUTHWEST) && north
				&& east)
			f = INVTOPSOUTHWEST;
		else if ((fNorth == TOPEAST || fNorth == TOPNORTHEAST)
				&& (fEast == TOPNORTH || fEast == TOPNORTHEAST)
				&& fSouth == NONE && fWest == NONE
				&& (!top || fTop == NORTHEAST || fTop == TOPNORTHEAST) && south
				&& west)
			f = INVTOPNORTHEAST;
		else if ((fSouth == TOPEAST || fSouth == TOPSOUTHEAST)
				&& (fEast == TOPSOUTH || fEast == TOPSOUTHEAST)
				&& fNorth == NONE && fWest == NONE
				&& (!top || fTop == SOUTHEAST || fTop == TOPSOUTHEAST) && north
				&& west)
			f = INVTOPSOUTHEAST;
	...
	public boolean shouldConnect(World world, int x, int y, int z, ForgeDirection side)
	{
		...
		return (blockID == this.blockID || isShapedBlock(blockID) || world
	...
	public boolean isShapeSolid(int metadata, ForgeDirection side)
	{
		if (side == ForgeDirection.DOWN)
			return metadata == TOPNORTH || metadata == TOPSOUTH
					|| metadata == TOPWEST || metadata == TOPEAST;

		if (side == ForgeDirection.NORTH
				&& (metadata == TOPSOUTH || metadata == SOUTHEAST || metadata == SOUTHWEST))
			return true;
		if (side == ForgeDirection.SOUTH
				&& (metadata == TOPNORTH || metadata == NORTHEAST || metadata == NORTHWEST))
			return true;
		if (side == ForgeDirection.WEST
				&& (metadata == TOPEAST || metadata == NORTHEAST || metadata == SOUTHEAST))
			return true;
		if (side == ForgeDirection.EAST
				&& (metadata == TOPWEST || metadata == NORTHWEST || metadata == SOUTHWEST))
			return true;
	...
	{
		return blockID == MalisisBetterTerrain.shapedGrass.blockID
				|| blockMap.get(blockID) == MalisisBetterTerrain.shapedGrass.blockID;
	}
	...
	{
		return originalBlock.onBlockActivated(world, x, y, z, p, par6, par7,
				par8, par9);
	}
	...
	{
		if (!this.shaped)
			return new AxisAlignedBB[] { AxisAlignedBB.getBoundingBox(0, 0, 0,
					1, 1, 1) };
	...
	public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 start, Vec3 end)
	{
		...
		for (AxisAlignedBB a : aabb)
		{
			setBlockBounds((float) a.minX, (float) a.minY, (float) a.minZ,
